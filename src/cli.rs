...
pub enum CallKind {
    ...
    #[structopt(
        name = "loh",
        about = "Call loss-of-heterozygosity regions based on variant calls with events `loh` and `no_loh`. This is experimental (do not use it yet).",
        setting = structopt::clap::AppSettings::ColoredHelp,
    )]
    LOH {
        #[structopt(
            ...
            required = true,
            long,
            help = "VCF/BCF file to process, generated by varlociraptor with a `schema.yaml` \
                    specifying `loh`, `no_loh` and events for the remaining event space."
        )]
        calls: PathBuf,
        #[structopt(
            parse(from_os_str),
