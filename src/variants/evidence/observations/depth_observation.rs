...
pub(crate) fn expected_depth(obs: &[ProcessedDepthObservation]) -> u32 {
    LogProb::ln_sum_exp(&obs.iter().map(|o| o.prob_mapping).collect_vec())
        .exp()
        .round() as u32
}

pub type ProcessedDepthObservation = DepthObservation<ReadPosition, AltLocus>;

impl<P: Clone, A: Clone> DepthObservationBuilder<P, A> {
...
impl DepthObservation<Option<u32>, ExactAltLoci> {
    pub(crate) fn process(
        &self,
        major_read_position: Option<u32>,
        major_alt_locus: &Option<genome::Locus>,
        alignment_properties: &AlignmentProperties,
    ) -> DepthObservation<ReadPosition, AltLocus> {
        DepthObservation {
...
impl<P: Clone, A: Clone> DepthObservation<P, A> {
...
impl ProcessedDepthObservation {
...
pub(crate) fn adjust_singleton_evidence(pileups: &mut [Pileup]) -> bool {
    let mut alt_observations: Vec<&mut DepthObservation<_, _>> = pileups
        .iter_mut()
        .flat_map(|pileup| pileup.depth_observations_mut().iter_mut())
...
pub(crate) fn major_read_position(
    pileup: &[DepthObservation<Option<u32>, ExactAltLoci>],
) -> Option<u32> {
...
pub struct DepthObservation<P = Option<u32>, A = ExactAltLoci>
where
    P: Clone,
    A: Clone,
{
