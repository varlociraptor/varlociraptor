...
const BWA_MAPQ_COEF_LEN: u32 = 50;
const BWA_MATCH_SCORE: u32 = 1;
const BWA_MISMATCH_PENALTY: u32 = 4;

/// Re-calculate mapping probability considering only single read
pub fn prob_mapping_se(
    record: &bam::Record
) -> LogProb {
    lazy_static! {
        // regex for a cigar string operation
        static ref XA_ENTRY: Regex = Regex::new(
            "(?P<chrom>[^,]+),[+-]?(?P<pos>[0-9]+),(?P<cigar>([0-9]+[MIDNSHP=X])+),(?P<dist>[0-9]+);"
        ).unwrap();
    }

    if let (Some(score), Some(sub_score)) = (record.aux(b"AS"), record.aux(b"XS")) {
        let score = score.integer();
        let sub_score = sub_score.integer();
        let mapq = if sub_score > score {
            0
        } else {
            let (qlen, rlen) = alignment_len(record);
            let alen = cmp::max(qlen, rlen);
            // mapq coef factor as defined in BWA: https://github.com/lh3/bwa/blob/5961611c358e480110793bbf241523a3cfac049b/bwamem.c#L957
            let mapq_coef_fac = if alen < BWA_MAPQ_COEF_LEN {
                1.0
            } else {
                (BWA_MAPQ_COEF_LEN as f64).ln()
            };

            let identity = 1.0 - (alen as i64 * BWA_MATCH_SCORE as i64 - score) as f64 /
                           (BWA_MATCH_SCORE + BWA_MISMATCH_PENALTY) as f64 / alen as f64;
            println!("identity {}", identity);
            println!("read {}", str::from_utf8(record.qname()).unwrap());

            let numerator = 6.02 * (score - sub_score) as f64;
            let denominator = BWA_MATCH_SCORE as f64 * (mapq_coef_fac * identity.powi(2)).powi(2) + 0.499;
            let mut mapq = (numerator / denominator) as i64;
            if let (Some(alt_alignments), Some(dist)) = (record.aux(b"XA"), record.aux(b"NM")) {
                let dist = dist.integer();
                let n_alt = XA_ENTRY.captures_iter(
                    str::from_utf8(alt_alignments.string()).unwrap()
                ).fold(0, |count, m| {
                    let alt_dist = i64::from_str_radix(&m["dist"], 10).expect("error parsing XA tag");
                    if alt_dist <= dist {
                        count + 1
                    } else {
                        count
                ...
                // let n_alt =  alt_alignments.string().iter().fold(
                //     0, |count, c| count + if *c == b';' { 1 } else { 0 }
                // );
                println!("n_alt {} mapq {}", n_alt, mapq);
                if n_alt > 0 {
                    mapq -= (4.343 * ((n_alt + 1) as f64).ln() + 0.499) as i64;
                }
            }
            mapq = cmp::max(cmp::min(mapq, 60), 0);
            mapq = (mapq as f64 + 0.499) as i64;
            mapq
        };
        println!("mapq {}", mapq);
