...
pub fn prob_snv(
    record: &bam::Record,
    cigar: &CigarStringView, // TODO: remove this parameter, passing in the record is enough and removes the necessity of copying over the cigar string before
    start: u32,
    variant: &Variant,
    ref_seq: &[u8]
) -> Result<(LogProb, LogProb), Box<Error>> { // TODO: needs to be able to return None, when variant position is where the CigarString only consumes reference ('D' - Deletion, 'N' - RefSkip), as those reads are not filtered out before but don't have a read base covering the SNV
